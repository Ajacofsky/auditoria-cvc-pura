...
# ==========================================
# GENERADOR DE PDF (DISEÑO FOTOGRÁFICO LIMPIO Y SEGURO)
# ==========================================
def generar_pdf_moderno(incap_od, grados_od, img_od_orig, incap_oi, grados_oi, img_oi_orig, incap_total, modo):
    ...
    pdf.ln(8) # Espacio para respirar

    # 2. IMÁGENES ORIGINALES (Reducidas y centradas)
    y_images = pdf.get_y()
    ...
    if modo == "Bilateral (OD y OI)":
        if img_od_orig is not None:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp_od:
                ...
                # Ojo Derecho: 90mm de ancho (más chico)
        ...
        if img_oi_orig is not None:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp_oi:
                ...
                # Ojo Izquierdo: 90mm de ancho
                pdf.image(tmp_oi.name, x=110, y=y_images, w=90) 
            os.remove(tmp_oi.name)
        # Salto vertical GIGANTE garantizado (95mm hacia abajo) para que no tape nada
        ...
        if img_val is not None:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                ...
                # Una sola imagen: 110mm de ancho y perfectamente centrada
                pdf.image(tmp.name, x=50, y=y_images, w=110) 
            os.remove(tmp.name)
        # Salto vertical garantizado (115mm hacia abajo)
        pdf.set_y(y_images + 115)

    # 3. RESULTADOS MATEMÁTICOS (Ahora siempre quedarán abajo)
